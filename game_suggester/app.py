...
def model_candidates(snapshot: Dict[str, Any], model: str, max_candidates: int) -> Tuple[List[Candidate], str]:
...
def suggest(
    ...
    model_used = True
    try:
        model_cands, model_err = model_candidates(snapshot, model=model, max_candidates=max_candidates)
